...
export async function deleteWave(id: string, path: string): Promise<void> {
  try {
    ...
    const descendantWaveIds = [id, ...descendantWaves.map((Wave) => Wave._id)];
      ...
      .populate({
        ...
        populate: [
          ...
          },
          {
            ...
            model: Wave, // The model of the nested children (assuming it's the same "Thread" model)
            populate: {
